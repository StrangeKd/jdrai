...
export function useGameSession(adventureId: string, options?: { isNew?: boolean; isResume?: boolean }): GameSessionState {
  ...
  const ui = useGameUI({ adventureId, options, onConfirmExitSave: manualSave, navigate, gameState });
  ...
  const { triggerSave: _triggerSave, showMilestone: _showMilestone, handleIntroEnd: _handleIntroEnd, ...uiPublic } = ui;
